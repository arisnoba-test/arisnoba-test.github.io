...
var f_wbFnUnitReg = new wbFnUnitReg();

function wbFnUnitReg(){
	// 함수 선언
	var t_wbFnUnitReg = this;
	
	// 게시물 목록 가져오기
	this.fnSearchPage = function(){
		// 등록 완료 시 첫페이지로 이동함.
		var objForm = document.frm;
		$("#btn_wb_f_current_page").val(1);
	...
	this.fnReg = function(){
		...
		var option = {
				url: "wb.insertUnit.do",
				callbackFn: t_wbFnUnitReg.fnCallback,
	...
	this.fnCallback = function(){
		alert("등록 완료했습니다.");
		f_wbFnUnitReg.fnSearchPage();
	};
	
	// 게시물 등록
	this.fnFileReg = function(){
	...
	// 등록 후 콜백 처리
	this.fnFileCallback = function(data, state){
		alert("11111");
		if (data=="error"){
		   alert("파일전송중 에러 발생!!");
		   return false;
